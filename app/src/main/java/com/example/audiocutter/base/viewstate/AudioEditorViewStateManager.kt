...
interface AudioEditorViewStateManager {
    ...
    fun editorSaveMixingAudio(
        ...
        if (getViewStateMutable().getLastState() == ViewStateScreen.MIXING_EDITOR_SCREEN) {
            ...
            val audioPathArray = arrayOf(
                audioFile1.file.absolutePath.toString(),
                audioFile2.file.absolutePath.toString()
            )


            val action = MixingScreenDirections.goToMixingEditorResultScreen(
                ResultScreen.MIX,
                audioPathArray,
                audioMixConfig,
                null,
    ...
    fun editorSaveCutingAudio(
        ...
        if (getViewStateMutable().getLastState() == ViewStateScreen.CUTTING_EDITOR_SCREEN) {
            getViewStateMutable().pushViewState(ViewStateScreen.RESULT_SCREEN)
            val audioPathArray = arrayOf(
                audioFile.file.absolutePath.toString()
            )


            val action = CuttingEditorScreenDirections.goToCuttingEditorResultScreen(
                ResultScreen.CUT,
                audioPathArray,
                null,
                audioCutConfig,
    ...
    fun editorSaveMergingAudio(
        baseFragment: BaseFragment,
        listPathAudio: List<AudioCutterView>,
        audioMergingConfig: AudioMergingConfig
    ) {
        if (getViewStateMutable().getLastState() == ViewStateScreen.MERGING_EDITOR_SCREEN) {
            ...
            val action = MergePreviewScreenDirections.goToMergingEditorResultScreen(
                ResultScreen.MER,
                audioPathArray.toTypedArray(),
                null,
                null,
                audioMergingConfig
            )

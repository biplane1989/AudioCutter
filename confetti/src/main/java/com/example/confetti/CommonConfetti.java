...
import com.example.confetti.confetto.BalloonBitmapConfetto;
import com.example.confetti.confetto.BitmapConfetto;
import com.example.confetti.confetto.Confetto;
import com.example.confetti.confetto.StarBitmapConfetto;
...
public class CommonConfetti {
    ...
    private static int defaultVelocityVeryFast;
    ...
    /**
     * Configures a confetti manager that has confetti star exploding out in all directions from the
     * provided x and y coordinates.
     *
     * @param container the container viewgroup to host the confetti animation.
     * @param x         the x coordinate of the explosion source.
     ...
     * @return the created common confetti object.
     */
    public static CommonConfetti starExplosion(ViewGroup container, int x, int y) {
        final CommonConfetti commonConfetti = new CommonConfetti(container);
        commonConfetti.configureStarExplosion(container, x, y);
        return commonConfetti;
    }
     ...
     * @return the created common confetti object.
     */
    public static CommonConfetti balloonExplosion(ViewGroup container, int x, int y) {
        final CommonConfetti commonConfetti = new CommonConfetti(container);
        commonConfetti.configureBalloonExplosion(container, x, y);
        return commonConfetti;
    }
    ...
    private void configureStarExplosion(ViewGroup container, int x, int y) {
        final Context context = container.getContext();
        final ConfettoGenerator generator = new ConfettoGenerator() {
            @Override
            public Confetto generateConfetto(Random random) {
                return new StarBitmapConfetto(container.getContext().getApplicationContext());
            }
        };
        final ConfettiSource confettiSource = new ConfettiSource(x, y);
        confettiManager = new ConfettiManager(context, generator, confettiSource, container)
                .setTTL(1500)
                .setBound(new Rect(
                        container.getLeft(), container.getTop(),
                        container.getRight(), container.getBottom()
                ))
                .setEmissionDuration(500)
                .setEmissionRate(400)
                .setVelocityX(Utils.convertDp2Px(20, container.getContext().getApplicationContext()), defaultVelocityVeryFast)
                .setVelocityY(Utils.convertDp2Px(20, container.getContext().getApplicationContext()), defaultVelocityVeryFast)
                .enableFadeOut(Utils.getDefaultAlphaInterpolator())
                .setInitialScale(1f, 0.8f)
                .setScaleAcceleration(0.5f, 1f);
    }


    private void configureBalloonExplosion(ViewGroup container, int x, int y) {
        final Context context = container.getContext();
        final ConfettoGenerator generator = new ConfettoGenerator() {
            @Override
            public Confetto generateConfetto(Random random) {
                return new BalloonBitmapConfetto(container.getContext().getApplicationContext());
            }
        };
        final ConfettiSource confettiSource = new ConfettiSource(x, y);
        confettiManager = new ConfettiManager(context, generator, confettiSource, container)
                ...
                .setVelocityX(Utils.convertDp2Px(20, container.getContext().getApplicationContext()), defaultVelocityVeryFast)
                .setVelocityY(Utils.convertDp2Px(20, container.getContext().getApplicationContext()), defaultVelocityVeryFast)
                .enableFadeOut(Utils.getIncreaseDefaultAlphaInterpolator())
                .setInitialScale(1f, 0.8f)
                .setScaleAcceleration(0.5f, 1f);
        confettiManager.getConfettiView().setLayerType(View.LAYER_TYPE_SOFTWARE, null);
    }
    ...
    private static void ensureStaticResources(ViewGroup container) {
        if (defaultConfettiSize == 0) {
            ...
            defaultVelocityVeryFast = res.getDimensionPixelOffset(R.dimen.default_velocity_very_fast);

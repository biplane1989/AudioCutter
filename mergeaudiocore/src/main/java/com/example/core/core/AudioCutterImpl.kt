...
class AudioCutterImpl : AudioCutter {
    ...
    private val CMD_CUT_AUDIO_TIME = "-y -ss %f -i \'%s\' -map a -c copy -t %f -b:a %dk -af \"volume='(between(t,0,%f)*(t/%d)+between(t,%d,%f)+between(t,%d,%d)*((%d-t)/(%d-%d)))*%f'\":eval=frame \"%s\""
    private val CMD_CUT_AUDIO_TIME_NOT_COPY = "-y -ss %f -i \'%s\' -map a -t %f -b:a %dk -af \"volume='(between(t,0,%f)*(t/%d)+between(t,%d,%f)+between(t,%d,%d)*((%d-t)/(%d-%d)))*%f'\":eval=frame \"%s\""

    private val CMD_CUT_AUDIO_FADE_IN_OFF = "-y -ss %f -i \'%s\' -map a -c copy -t %f -b:a %dk -af \"volume='(between(t,%d,%f)+between(t,%d,%d)*((%d-t)/(%d-%d)))*%f'\":eval=frame \"%s\""
    private val CMD_CUT_AUDIO_FADE_IN_OFF_NOT_COPY = "-y -ss %f -i \'%s\' -map a -t %f -b:a %dk -af \"volume='(between(t,%d,%f)+between(t,%d,%d)*((%d-t)/(%d-%d)))*%f'\":eval=frame \"%s\""

    private val CMD_CUT_AUDIO_TIME_FADE_OUT_OFF = "-y -ss %f -i \'%s\' -map a -c copy -t %f -b:a %dk -af \"volume='(between(t,0,%f)*(t/%d)+between(t,%d,%f))*%f'\":eval=frame \"%s\""
    private val CMD_CUT_AUDIO_TIME_FADE_OUT_OFF_NOT_COPY = "-y -ss %f -i \'%s\' -map a -t %f -b:a %dk -af \"volume='(between(t,0,%f)*(t/%d)+between(t,%d,%f))*%f'\":eval=frame \"%s\""

    private val CMD_CUT_AUDIO_TIME_FADE_OFF = "-y -ss %f -i \'%s\' -map a -c copy -t %f -b:a %dk -af \"volume='(1*%f)'\":eval=frame \"%s\""
    private val CMD_CUT_AUDIO_TIME_FADE_OFF_NOT_COPY = "-y -ss %f -i \'%s\' -map a -t %f -b:a %dk -af \"volume='(1*%f)'\":eval=frame \"%s\""

    private val CMD_CONCAT_AUDIO = "-y %s -filter_complex \"concat=n=%d:v=0:a=1[a]\" -map \"[a]\" -b:a %dk \"%s\""
    private val CMD_MIX_AUDIO = "-y -i \'%s\' -i \'%s\' -filter_complex \"[0:0]volume=%f[a];[1:0]volume=%f[b];[a][b]amix=inputs=2:duration=%s:dropout_transition=0[a]\" -map \"[a]\" -q:a 0 \"%s\""
...
//                }
    ...
    private fun getStringFormat(audioCutConfig: AudioCutConfig, audioFile: AudioCore, fileCutPath: String): String {
        ...
        if (audioCutConfig.inEffect != Effect.OFF && audioCutConfig.outEffect != Effect.OFF) {
            return String.format(Locale.ENGLISH, if (audioFile.bitRate != audioCutConfig.bitRate.value) CMD_CUT_AUDIO_TIME_NOT_COPY else CMD_CUT_AUDIO_TIME, audioCutConfig.startPosition, audioFile.file.absolutePath, audioCutConfig.endPosition, audioCutConfig.bitRate.value, if (audioCutConfig.inEffect.time == 0) 0f else (audioCutConfig.inEffect.time - 0.0001).toFloat(), audioCutConfig.inEffect.time, audioCutConfig.inEffect.time, ((audioCutConfig.endPosition.toInt() - audioCutConfig.outEffect.time) - 0.0001).toFloat(), audioCutConfig.endPosition.toInt() - audioCutConfig.outEffect.time, audioCutConfig.endPosition.toInt(), audioCutConfig.endPosition.toInt(), audioCutConfig.endPosition.toInt(), (audioCutConfig.endPosition.toInt() - audioCutConfig.outEffect.time), audioCutConfig.volumePercent / 100f, fileCutPath)
        } else if (audioCutConfig.inEffect == Effect.OFF && audioCutConfig.outEffect == Effect.OFF) {
            return String.format(Locale.ENGLISH, if (audioFile.bitRate != audioCutConfig.bitRate.value) CMD_CUT_AUDIO_TIME_FADE_OFF_NOT_COPY else CMD_CUT_AUDIO_TIME_FADE_OFF, audioCutConfig.startPosition, audioFile.file.absolutePath, audioCutConfig.endPosition, audioCutConfig.bitRate.value, audioCutConfig.volumePercent / 100f, fileCutPath)
        } else if (audioCutConfig.inEffect == Effect.OFF) {
            return String.format(Locale.ENGLISH, if (audioFile.bitRate != audioCutConfig.bitRate.value) CMD_CUT_AUDIO_FADE_IN_OFF_NOT_COPY else CMD_CUT_AUDIO_FADE_IN_OFF, audioCutConfig.startPosition, audioFile.file.absolutePath, audioCutConfig.endPosition, audioCutConfig.bitRate.value, 0, ((audioCutConfig.endPosition.toInt() - audioCutConfig.outEffect.time) - 0.0001).toFloat(), audioCutConfig.endPosition.toInt() - audioCutConfig.outEffect.time, audioCutConfig.endPosition.toInt(), audioCutConfig.endPosition.toInt(), audioCutConfig.endPosition.toInt(), (audioCutConfig.endPosition.toInt() - audioCutConfig.outEffect.time), audioCutConfig.volumePercent / 100f, fileCutPath)
        } else {
            return String.format(Locale.ENGLISH, if (audioFile.bitRate != audioCutConfig.bitRate.value) CMD_CUT_AUDIO_TIME_FADE_OUT_OFF_NOT_COPY else CMD_CUT_AUDIO_TIME_FADE_OUT_OFF, audioCutConfig.startPosition, audioFile.file.absolutePath, audioCutConfig.endPosition, audioCutConfig.bitRate.value, if (audioCutConfig.inEffect.time == 0) 0f else (audioCutConfig.inEffect.time - 0.0001).toFloat(), audioCutConfig.inEffect.time, audioCutConfig.inEffect.time, audioCutConfig.endPosition, audioCutConfig.volumePercent / 100f, fileCutPath)
        }
    }
    ...
    override suspend fun mix(audioFile1: AudioCore, audioFile2: AudioCore, audioMixConfig: AudioMixConfig): AudioCore {
        withContext(Dispatchers.Default) {
            ...
            val requestCode = FFmpeg.execute(String.format(Locale.ENGLISH, CMD_MIX_AUDIO, audioFile1.file.absolutePath, audioFile2.file.absolutePath, (audioMixConfig.volumePercent1 / 100f), (audioMixConfig.volumePercent2 / 100f), audioMixConfig.selector.type, filePath))
    ...
    override suspend fun merge(listAudioFile: List<AudioCore>, fileName: String, audioFormat: AudioFormat, pathFolder: String): AudioCore {
        ...
        withContext(Dispatchers.Default) {
            ...
            val mimeType = if (audioFormat == AudioFormat.MP3) AudioFormat.MP3.type else AudioFormat.AAC.type
            ...
            val returnCode = FFmpeg.execute(String.format(Locale.ENGLISH, CMD_CONCAT_AUDIO, cmd_input, sizeAudioFile, bitRateFile, pathFileMerge))

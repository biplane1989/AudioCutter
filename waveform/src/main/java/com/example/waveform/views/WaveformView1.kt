...
class WaveformView1 : View, ProgressListener {
    ...
    private fun init(context: Context) {
        ...
        val centerLineWidth = dpToPx(context, CENTER_LINE_WIDTH)
        ...
        playLineWidth = dpToPx(getContext(), DEFAULT_PLAY_LINE_WIDTH_DP)
        ...
        cursorSize = dpToPx(getContext(), DEFAULT_CURSOR_SIZE_DP)
        ...
        timeMarkPaint!!.textSize = spToPx(
            getContext(),
            DEFAULT_TIME_TEXT_SIZE
        )
        timeMarkTextMargin = dpToPx(getContext(), DEFAULT_TIME_TEXT_MARGIN)
    ...
    fun setDataSource(filePath: String, duration: Long) {
        if (listener != null) {
    ...
    private fun initDimensions() {
        ...
        cursorLeftRect!![0f, dpToPx(context, 32f), cursorSize] =
            cursorSize + dpToPx(context, 32f)
        cursorRightRect!![widthView - cursorSize, heightView - cursorSize - dpToPx(
            context,
            32f
        ), widthView.toFloat()] =
            heightView.toFloat() - dpToPx(context, 32f)
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (w != oldw || h != oldh) {
            if (isReadyToDraw() && mAudioDecoder != null) {
                mWaveformDrawer.computeDoublesForAllZoomLevels(mAudioDecoder)
            }
            widthView = w
            heightView = h
    ...
    private fun drawTimeMark(canvas: Canvas) {
        if (trackDurationMs > 0 && timeRange > 0) {
            ...
            while (i < stopX) {
                ...
                if (x >= 0) {
                    val fl = getWidthText(longMsToString(i), context) / 2
                    canvas.drawText(
                        longMsToString(i),
...
//                    )
    ...
    private fun drawSelectCursors(canvas: Canvas) {
        ...
        canvas.drawText(
            longDurationMsToStringMs((cursorLeftPortion * trackDurationMs).toLong()),
            selectRect!!.left,
            selectRect!!.top - timeMarkTextMargin,
            timeMarkPaint!!
        )
        canvas.drawText(
            longDurationMsToStringMs((cursorRightPortion * trackDurationMs).toLong()),
            selectRect!!.right,
            selectRect!!.top - timeMarkTextMargin,
            timeMarkPaint!!
        )
...
//        canvas.drawRect(selectRect!!, selectRectPaint!!)
    ...
    private fun changeSelectRange(event: MotionEvent) {
        if (selectingCursor == 1) {
            if (event.x - cursorSize / 2 >= 0 && event.x + cursorSize / 2 <= cursorRightRect!!.right) {
                cursorLeftRect!![event.x - cursorSize / 2, +dpToPx(
                    context,
                    32f
            ...
            if (event.x + cursorSize / 2 <= widthView && event.x - cursorSize / 2 >= cursorLeftRect!!.left) {
                cursorRightRect!![event.x - cursorSize / 2, heightView - cursorSize - dpToPx(
                    context,
                    32f
                ), event.x + cursorSize / 2] =
                    heightView.toFloat() - dpToPx(
                        context,
                        32f
                    )
    ...
    private fun updateCursors() {
        ...
        cursorLeftRect!![cursorLeftX, +dpToPx(context, 32f), cursorLeftX + cursorSize] =
            cursorSize + dpToPx(context, 32f)
    ...
    companion object {
        val TAG = WaveformView1::class.java.simpleName
